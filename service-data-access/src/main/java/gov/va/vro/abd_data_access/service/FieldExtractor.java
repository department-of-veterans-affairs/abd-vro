    ...
    public static AbdMedication extractMedication(MedicationRequest medication) {
        ...
        if (medication.hasNote()) {
            result.setNotes(medication.getNote().stream()
                    ...
                    .map(Annotation::getText).collect(Collectors.toList()));
        }
        if (medication.hasDosageInstruction()) {
            List<String> dosages = medication.getDosageInstruction()
                    .parallelStream().filter(Dosage::hasText)
                    .map(Dosage::getText).collect(Collectors.toList());
            List<String> codeText = medication.getDosageInstruction().stream()
                    .filter(Dosage::hasTiming).filter(t -> t.getTiming().hasCode())
            ...
            String routes = medication.getDosageInstruction().stream()
                    .filter(d -> d.hasRoute() && d.getRoute().hasText())
                    .map(d -> d.getRoute().getText())
                    .findFirst().orElse(""); // Take 1st one for now
