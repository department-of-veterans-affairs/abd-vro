...
def on_generate_fetch_callback(channel, method, properties, body):
    try:
        redis_client = RedisClient(redis_config)
        pdf_generator = PDFGenerator(pdf_options)
...
def queue_setup(channel):
    ...
    channel.basic_consume(queue=GENERATE_FETCH_QUEUE, on_message_callback=on_generate_fetch_callback, auto_ack=True)
