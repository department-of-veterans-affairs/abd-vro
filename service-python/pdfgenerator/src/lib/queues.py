...
from .settings import (
    codes as DIAGNOSTIC_CODE_MAPPING,
    pdf_options,
    redis_config,
    queue_config,
)
    ...
    try:
        ...
        if redis_client.exists(claim_id):
            ...
            response = {
                ...
                "status": "COMPLETE",
                "diagnosis": str(diagnosis_name.decode("ascii")),
                "pdfData": str(pdf.decode("ascii")),
            }
        else:
...
def queue_setup(channel):
    ...
    channel.basic_consume(
        queue=GENERATE_QUEUE, on_message_callback=on_generate_callback, auto_ack=True
    )
    # Fetch PDF Queue
    channel.queue_declare(queue=FETCH_QUEUE)
    channel.queue_bind(queue=FETCH_QUEUE, exchange=EXCHANGE)
    channel.basic_consume(
        queue=FETCH_QUEUE, on_message_callback=on_fetch_callback, auto_ack=True
    )
    logging.info(
        f" [*] Waiting for data for queue: {GENERATE_QUEUE}. To exit press CTRL+C"
    )
    logging.info(
        f" [*] Waiting for data for queue: {FETCH_QUEUE}. To exit press CTRL+C"
    )
