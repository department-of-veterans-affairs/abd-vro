...
def test_valid_queue_created():
    ...
    generate_queue_exists = consumer.channel.queue_declare(queue=consumer_settings["generate_queue_name"], durable=True, passive=True)
    fetch_queue_exists = consumer.channel.queue_declare(queue=consumer_settings["fetch_queue_name"], durable=True, passive=True)
    assert generate_queue_exists
    assert fetch_queue_exists