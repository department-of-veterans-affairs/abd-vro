...
describe MetricLogger do
  ...
  it "generates a metric intake payload" do
    ...
    expect(payload.series[0].tags).to match_array(["sunny:true", "humid:false", "environment:local", "service:vro-svc-bgs-api"])
  ...
  it "generates a distribution metric payload" do
    timestamp_before_getting_payload = Time.now.to_i
    payload = client.generate_distribution_metric(METRIC[:REQUEST_DURATION],
        Time.now.to_i, 424, ["dog.adoptions.ventura"])

    expect(payload.series.length).to eq(1)
    expect(payload.series[0].metric).to eq("vro_bgs.request_duration")
    expect(payload.series[0].tags).to match_array(["dog.adoptions.ventura", "environment:local", "service:vro-svc-bgs-api"])
    expect(payload.series[0].points.length).to eq(1)
