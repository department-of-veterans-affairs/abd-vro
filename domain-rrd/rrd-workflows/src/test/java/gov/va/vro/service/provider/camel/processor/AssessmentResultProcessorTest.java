...
public class AssessmentResultProcessorTest {
  ...
  private static final String diagnosticCode = "7101";
  private static final UUID claimId = UUID.randomUUID();

  private static final AbdEvidenceWithSummary evidence = new AbdEvidenceWithSummary();
  ...
  void testMissingClaimId() {
    ...
    Mockito.verify(saveToDbService, Mockito.never())
        .insertAssessmentResult(Mockito.any(), Mockito.any(), Mockito.any());
  ...
  void testSuccessfulSave() {
    ...
    Mockito.verify(saveToDbService, Mockito.times(1))
        .insertAssessmentResult(claimId, evidence, diagnosticCode);
  }
}
